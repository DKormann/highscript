...
mutual
  ...
  inductive Adt : {name: String} -> Type
    | nil : Adt
    | cons : Variant -> @Adt name -> @Adt name
  deriving BEq, Hashable, DecidableEq

  inductive Variant : {name: String} -> Type
    | nil : Variant
    | Rcons : @Variant name -> @Variant name
    | cons : Ty -> @Variant name  -> @Variant name
  deriving BEq, Hashable, DecidableEq
...
def Variant.name {name} (v: @Variant name) : String := name


structure Nvariant where
  name:String
  variant: @Variant name
deriving BEq, Hashable, DecidableEq

def Variant.Nv {name} (v:@Variant name) := Nvariant.mk v.name v

open Ty

mutual

  def Adt.repr : @Adt name → String
    | .nil => "nil"
    | .cons v a => s!".cons\{{v.repr}} {a.repr}"

  def Variant.repr : @Variant name → String
    | .nil => "nil"
    | .Rcons v => s!"R {v.repr}"
    | .cons t v => s!"T {t.repr} {v.repr}"
  ...
  instance : Repr (@Adt name) where reprPrec adt _ := adt.repr
  instance : Repr (@Variant name) where reprPrec v _ := v.repr
  instance : Repr Ty where reprPrec t _ := t.repr

  instance : ToString (@Adt name) where toString (a:Adt) := a.repr
  instance : ToString (@Variant name) where toString (a:Variant) := a.repr
  instance : ToString Ty where toString (a:Ty) := a.repr

  def Variant.fields (self:Ty) : @Variant name -> List Ty
    | .nil => []
    | .Rcons v => self :: v.fields self
    | .cons t v => t::v.fields self

  @[simp, reducible]
  def Adt.variants : @Adt name -> List Nvariant
    | .nil => []
    | .cons v a => (@Nvariant.mk v.name v)::a.variants
...
def nil : @Variant "nil" := .nil
def cons : @Variant "cons" := .cons int $ .Rcons .nil
def leaf : @Variant "leaf" := .cons int .nil
def sleaf : @Variant "sleaf" := .cons string .nil
def list : @Adt "list" := .cons cons $ .cons nil $ .nil
...
mutual
  ...
  inductive Expr : Ty → Type
    ...
    | data : {a:Adt} -> {v:Variant} -> Instance a v ->
      (p: v.Nv ∈ a.variants := by unfold Adt.variants; simp)->
      Expr (Ty.adt a)

  inductive Instance : Adt -> Variant -> Type
    | nil : Instance a nil
    | self : (e: Expr $ .adt t) -> Instance t v -> Instance t (.Rcons v)
    | cons : (e: Expr a) -> Instance t v -> Instance t (.cons a v)

  inductive Match : (a:Adt) -> (done: Adt) -> (res:Ty) -> Type
    | nil : Match arg Adt.nil res
    | cons : (name:String) -> MatchCase arg v res -> Match a xs res-> Match arg (Adt.cons v xs) res

  inductive MatchCase : (arg : Adt) -> (done: Variant) -> (res:Ty) -> Type
    | nil : Expr res -> MatchCase arg done res
    | self : (v:Var arg) -> MatchCase arg xs res -> MatchCase arg (Variant.Rcons xs) res
...
inductive Hol : List String -> Type
  | nil : Hol ["cons", "nil", "leaf"]
  | cons: (v:String) -> Hol (vs) -> Hol (vs.filter (fun x => x != v))


#check
  let h := Hol.nil
  let h : Hol ["nil","leaf"] := Hol.cons "cons" h
...
#check
  ...
  let lty : Ty := .adt "list" list



  let nn : Match lty .nil int := Match.nil

  let lperm := Adt.cons "nil" nil $ Adt.cons "cons" cons $ Adt.nil
  ...
  let mm
    ...
    Match.cons "cons"
      (MatchCase.cons (.mk "h") $
      MatchCase.self (.mk "t") $ MatchCase.nil $ .int 22)
      $
      Match.cons "nil" (MatchCase.nil $ .int 33)
      Match.nil
  ...
  let pm
    ...
    Match.cons "nil" (MatchCase.nil $ .int 33)
    $
    Match.cons "cons" (MatchCase.cons (.mk "h") $ MatchCase.self (.mk "t") $ MatchCase.nil $ .int 22)
    Match.nil

  let p : cons ∈ list.variants := by
    unfold list Adt.variants
    simp
