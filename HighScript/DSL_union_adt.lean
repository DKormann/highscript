...
-- def Variant.repr : Variant → String | (name, fields) => s!"{name}\{{" ".intercalate $ fields.map fun x => (x.choose "x" "rec")}}"
...
-- def Adt.repr (adt: Adt) : String := s!"data {adt.name} \{{" ".intercalate $ adt.Variants.map (Variant.repr)}}"
