...
#eval
  ...
  let ladt (a) : Adt := Adt.mk [
    .mk "Cons" (.cons (some a) $ .cons .none .nil),
    .mk "Nil" .nil,
  ]

  let nlist := NamedAdt.mk "list" (ladt int)
  let cn : Expr nlist.Ty := Expr.data 1 Instance.nil

  let cc : Expr nlist.Ty := Expr.data 0 $ Instance.cons (Expr.int 22) $ Instance.cons cn Instance.nil
  let cc : Expr nlist.Ty := Expr.data 0 $ Instance.cons (Expr.int 22) $ Instance.cons cc Instance.nil

  let aa :
    Match (nlist) (nlist.adt.variants.map (NamedVariant.name)) (nlist.adt.variants.map (NamedVariant.v)) int
    :=
      Match.cons "Cons"
  ...
  let ax : Expr int := Expr.mmatch cc aa

  let amat : Expr int := ~ cc : {
    #Nil {} : .int 2
    #Cons {x tail} : .int 22
  }
