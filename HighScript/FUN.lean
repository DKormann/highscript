...
inductive IMPLit : IMPType → Type
  | nat  : Nat  → IMPLit IMPType.nat
  | bool : Bool → IMPLit IMPType.bool
  | string : String → IMPLit IMPType.string

inductive IMPUnOp : IMPType -> IMPType → Type
  | not : IMPUnOp IMPType.bool IMPType.bool

inductive IMPBinOp : IMPType → IMPType → Type
  | add : IMPBinOp IMPType.nat IMPType.nat
  | and : IMPBinOp IMPType.bool IMPType.bool
  | less : IMPBinOp IMPType.nat IMPType.bool



inductive Var : IMPType -> Type
  | mk : String → (x:IMPType) → Var x

inductive IMPExpr : IMPType -> Type
  | lit : IMPLit x → IMPExpr x
  | var : Var u → IMPExpr u
  | un  : IMPUnOp a b → IMPExpr a → IMPExpr b
  | bin : IMPBinOp a b → IMPExpr a → IMPExpr a → IMPExpr b
  | lett : Var a → IMPExpr a → IMPExpr b → IMPExpr b

def getTy  (_: IMPExpr u) : IMPType := u

inductive IMPProgram

  | Assign : String → IMPExpr u → IMPProgram → IMPProgram

  | If     : IMPExpr bool → IMPProgram → IMPProgram → IMPProgram
  | While  : IMPExpr bool → IMPProgram → IMPProgram
...
syntax "let" ident "=" imp_expr "in" imp_expr : imp_expr
...
partial def elabIMPExpr : Syntax → MetaM Expr
  ...
  | `(imp_expr| let $i:ident = $e₁:imp_expr in $e₂:imp_expr) => do
    let e₁ ← elabIMPExpr e₁
    -- let τ ← mkAppM ``getTy #[e₁] >>= whnf
    let τ ← mkAppM ``getTy #[e₁] >>= reduce
    withLocalDecl i.getId BinderInfo.default τ (fun _ => do
      let e₂ ← elabIMPExpr e₂
      let var ← mkAppM ``Var.mk #[mkStrLit i.getId.toString, τ]
      mkAppM ``IMPExpr.lett #[var, e₁, e₂]
    )
  | `(imp_expr| $x:ident ) => do
    let lctx ←  getLCtx ;
    ...
    let var ← mkAppM ``Var.mk #[mkStrLit x.getId.toString, τ]


    mkAppM ``IMPExpr.var #[var]
...
#reduce IMPExpr.lett (Var.mk "a" nat) (IMPExpr.lit $ IMPLit.nat 2) (IMPExpr.lit $ IMPLit.nat 3)

#reduce test_elabIMPExpr let a = 2 in 2

#check test_elabIMPExpr let a = 2 in let a = true in a

#check test_elabIMPExpr let a = 2 in a
