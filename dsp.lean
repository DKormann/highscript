...
def g : Expr int := ToExpr.make 22
def og: Expr (option int) := ToExpr.make (some 22)
def lg: Expr (list int) := ToExpr.make [22, 33, 44]
...
def eval  (x:Expr r) := match x with
  ...
  | Expr.mmatch i d b x => s!"~({eval i}) \{ #None:{22} #Some\{{b.name}}: {eval x}}"
  | _ => "not implemented"



notation "(" a "•" b ")" => Expr.app a (ToExpr.make b)
-- notation "λ(" a "→" b ")" => Expr.lam a (ToExpr.make b)
...
#eval
  ...
  let fn :Expr (arrow int int) := lam x (Expr.var x)
